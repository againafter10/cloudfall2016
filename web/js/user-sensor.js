...
function user_sensors() {
    ...
    jQuery.ajax({
        url:"http://localhost:8181/virtualsensor?user_email="+uid,
...
function addsensorforuser() {
    ...
    var session = document.getElementById("session"), vid;
    vid = session.getAttribute("data-email");

    jQuery.ajax({
        url:"http://localhost:8181/sensors?vendor_email="+vid,
        type:"GET",
        contentType:'application/json',
        dataType:"json",
        success:function(data) {
